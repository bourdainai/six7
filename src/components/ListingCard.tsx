      ...
      <div className={`group relative ${className}`}>
        ...
        >
          ...
          <div className={`${isCardListing ? 'aspect-[5/7]' : 'aspect-square'} bg-soft-neutral overflow-hidden mb-3 relative border border-divider-gray group-hover:border-foreground transition-all duration-fast`}>
            ...
            {/* Firm Price Badge - Moved to bottom right */}
            {listing.accepts_offers === false && (
              <div className="absolute bottom-2 right-2">
                <Badge variant="secondary" className="bg-background/90 backdrop-blur text-[10px] h-5 px-1.5">
                  Firm
                </Badge>
              </div>
            )}
            ...
            {showSaveButton && onSaveClick && (
              <div className="absolute top-2 right-2 z-10">
                <Button
                  ...
                  disabled={isSaving}
                  className={`h-8 w-8 rounded-full bg-background/50 backdrop-blur-sm hover:bg-background/80 transition-all ${isSaved ? "text-red-500" : "text-foreground/70"}`}
                >
                  <Heart
