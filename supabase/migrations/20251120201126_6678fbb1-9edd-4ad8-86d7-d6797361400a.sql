...
-- Add missing columns, full-text search, popularity tracking, and sync metadata

-- Create sync_source enum (with proper IF NOT EXISTS handling)
DO $$ BEGIN
...
-- Add all missing columns to pokemon_card_attributes
ALTER TABLE pokemon_card_attributes 
  ...
  ADD COLUMN IF NOT EXISTS rarity VARCHAR(100),
  ...
  ADD COLUMN IF NOT EXISTS tcgplayer_id VARCHAR(100),
  ADD COLUMN IF NOT EXISTS cardmarket_id VARCHAR(100),
