...
const withdrawSchema = z.object({
  amount: z.number().positive('Amount must be greater than 0').max(10000, 'Maximum withdrawal is £10,000'),
...
serve(async (req) => {
  ...
  try {
    ...
    const { amount, destination_account_id } = withdrawSchema.parse(body);
