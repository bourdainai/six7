 ...
 * Maps National Pokedex ID (dexId) to English names
 * Covers Gen 1-9 (1-1025)
 */
...
/**
 * Get English name for a Pokemon by its National Dex ID
 */
export function getEnglishName(dexId: number | number[] | undefined): string | null {
  if (!dexId) return null;
  
  // If it's an array, use the first dexId
  const id = Array.isArray(dexId) ? dexId[0] : dexId;
  
  return POKEMON_NAMES[id] || null;
}

/**
 * Check if a name contains Japanese characters
 */
